...
void gyro_lowpass(arma::vec3* samples, int length, int divider) {
    if (divider < 2) return;
    ...
    arma::vec3 out[3] = {samples[0], samples[1], samples[2]};
...
void gyro_upsample(arma::vec3* samples, int length_new, int multiplier) {
    ...
    for (int i = 0; i < length_new; ++i) {
        if ((i + half_mult) % multiplier) {
            samples[i] = arma::fill::zeros;
        } else {
            samples[i] = samples[i / multiplier + old_samples_base];
        }
    }

    gyro_lowpass(samples, length_new, multiplier * 4);
}

void gyro_decimate(arma::vec3* samples, int length, int divider) {
    if (divider < 2) return;
