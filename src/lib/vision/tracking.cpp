...
void TrackerImpl::InitCorners(const cv::Mat& img) {
    static constexpr double discard_treshold = 1e-3;
    int minDist = std::sqrt(img.rows * img.cols / 3 / max_corners_);

    cv::goodFeaturesToTrack(img, corners_, max_corners_, discard_treshold,
                            minDist);
...
TrackerImpl::Track(const cv::Mat& prev, const cv::Mat& cur) {
    ...
    cv::calcOpticalFlowPyrLK(
        prev, cur, corners_, new_corners, status, err, cv::Size(21, 21), 6,
